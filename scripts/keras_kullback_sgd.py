...
logger = logging.getLogger("Kersa_kullback_sgd")
...
#   print( "Usage: %s -i  input-file\n" % sys.argv[0]
   ...
   print("Usage: %s -k number of user" % sys.argv[0])
...
def main(inFile,outFile,numFold,numEpoch):
    ...
    for i, (train, test) in enumerate(kfold):
        logging.info("Start of fold num %i"%i)
        # create model
        model = Sequential()
        ...
        logging.info("Testing prediction power for run %i"%i)
        right=0
        wrong=0
        i=0
        while i< len(Y[test]):
            if int(rounded[i])!=int(Y[test][i]):
                wrong+=1
            else:
                right+=1
            i+=1
...
#printing out mean and std
    ...
    with open("../anagrafica_files/result_keras", "w") as fo:
