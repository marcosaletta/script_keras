...
#        return luigi.util.task_wraps(self.model)
    ...
    # def output(outFil, pid):
    #     #pid=luigi.Parameter()
    #     return luigi.LocalTarget(outFil), luigi.LocalTarget(outFil+'_eval'), luigi.LocalTarget(outFil+'_tup_'+str(pid))
...
#    def requires(self): return run()
    ...
    def run(self):
        ...
        pid=str(1)
        #pid=luigi.Parameter()
        logging.info("--START RUN TASK WITH PID %s"%str(pid))
...
#         fo_l, fo_eval_l, fo_tup_l=self.output(self.outFile,pid)
#         fo=fo_l.open('w')
...
#         fo_tup.write(str(tupPred))
#         fo.close()
#         fo_eval.close()
#         file_user.close()
#         #tupPred=luigi.Parameter()
#         fo_tup.close()
        logging.info("END OF TASK WITH PID %s"%str(pid))
